...
def personal_map(input_func, lst):
	new_lst = []
	for element in lst:
		new_lst.append(input_func(element))
	return new_lst
	# for element in lst:
	# 	yield input_func(element)
...
## You can test this out with any code you like here, and similar below other problems, but make sure to comment out any code that uses up the 
## iterator in order to pass the tests!
# student_tups_list = []
# for name in names:
# 	for sen in seniority:
# 		for program in programs_written:
# 			student_tups_list.append((name, sen, program))
...
# Use the Python map function on the programmers list you just created, in order to create an map instance iterator of numbers representing the productivity of each student.
## Save the map iterator in a variable called prod_iter.
